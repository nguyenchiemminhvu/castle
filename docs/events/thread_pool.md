...
`castle::events::thread_pool` is a fixed-capacity thread pool implemented on top of POSIX threads.
...
A task is stored as `castle::callbacks::function<void()>`. The queues contain only task-slot indices. This is intentional because `castle::container::ring_buffer` is constrained to trivially copyable and trivially destructible element types, while `function` owns callable state.
...
```cpp
using task_type = castle::callbacks::function<void()>;
```
...
- return exactly `void`;
- fit inside the configured `function` storage.
...
```cpp
castle::events::thread_pool<2U, 8U> pool;
...
```cpp
static_assert(
castle::events::thread_pool<4U, 16U>::thread_count() == 4U,
"Unexpected worker count"
);

static_assert(
castle::events::thread_pool<4U, 16U>::task_capacity() == 16U,
...
```cpp
castle::events::thread_pool<4U, 32U> pool;
```
...
`thread_pool` is intentionally a small fixed-capacity execution primitive. It does not provide:
