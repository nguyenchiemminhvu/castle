...
Inline storage size/alignment for each of the four `castle::callbacks::inplace_function<void()>`
...
`castle::callbacks::inplace_function` — no heap allocation, no virtual dispatch, and a plain
...
multi-subscriber fan-out (see "Design rationale: why `inplace_function`, not a registry" below).
