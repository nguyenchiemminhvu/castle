...
// -----------------------------------------------------------------------------
// event_dispatcher - compile-time, tag-keyed event dispatcher with
// value-owned callbacks.
//
// Configuration model:
//   The event set is fixed at compile time via a pack of event_tag_config<...>.
//   event_dispatcher consumes every field on event_tag_config:
//   event_tag, max_callback, signature, callback_storage_size and
//   callback_storage_alignment. Using the same event_tag_config type for both
//   flavours keeps a single, uniform configuration surface — the user can
//   swap delegate_dispatcher <-> event_dispatcher just by changing the
...
//   - Each event owns its own callbacks::function_registry<max_callback,
...
//
// Storage:
//   - A castle::tuple of per-tag callbacks::function_registry instances holds the
//     whole subscriber state — sized independently per event, no runtime
//     allocation.
//
// Subscriptions:
//   - register_callback<Tag>(callable) returns an
//     callbacks::subscription carrying a back-pointer to the correct
...
//       event_tag_config<TimerExpired,   8, void(std::uint32_t, std::uint32_t)>,
//       event_tag_config<VehicleStarted, 4, void(std::uint8_t)>,
...
private:
    ...
    // callbacks::function_registry's void-return static_assert.
    // -------------------------------------------------------------------------
    template <typename T>
    ...
    // -------------------------------------------------------------------------
    // Extract per-config traits. event_dispatcher uses every field:
    ...
    // -------------------------------------------------------------------------
    // Per-event registry type — one owning callbacks::function_registry sized
...
public:
    ...
    //
    // Accepts any callable convertible to the callbacks::function signature
    // declared by event_tag_config<Tag, N, Signature, ...>. The callback is
    // stored by value inside the registry's callbacks::function buffer, so
    ...
    //
    // Perfect-forwarded to callbacks::function_registry::invoke(Args...).
...
private:
    ...
    // Direct access to the callbacks::function_registry for a specific tag.
    template <typename Tag>
...
private:
    ...
    // its event_tag_config and OWNS its callbacks by value via callbacks::function's
...
#endif // CASTLE_EVENTS_EVENT_DISPATCHER_HPP
