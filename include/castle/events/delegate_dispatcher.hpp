...
// -----------------------------------------------------------------------------
// delegate_dispatcher - compile-time, tag-keyed event dispatcher (non-owning).
//
// Configuration model:
//   The event set is fixed at compile time via a pack of event_tag_config<...>.
//   delegate_dispatcher only consumes event_tag, max_callback and signature from
//   each config; callback_storage_size / callback_storage_alignment are
//   event_dispatcher's concern and are ignored here. Using the same
//   event_tag_config type for both flavours keeps a single, uniform configuration
//   surface.
//
// Design:
//   - Each event is identified by a TAG TYPE (event_tag_config::event_tag). The
...
//   - Each event owns its own callbacks::delegate_registry<max_callback, signature>.
//     Callbacks are non-owning callbacks::delegate_base<Args...>* pointers, so any variant
...
//
// Storage:
//   - A castle::tuple of per-tag callbacks::delegate_registry instances holds the whole
//     subscriber state — sized independently per event, no runtime allocation.
//
// Subscriptions:
//   - register_callback<Tag>(&cb) returns a callbacks::subscription
...
//
//   using dispatcher_t = delegate_dispatcher<
...
//   castle::callbacks::delegate_ptr<void(uint32_t, uint32_t)> cb(&on_timer);
//
...
private:
    ...
    // -------------------------------------------------------------------------
    // Reject anything that is not an event_tag_config<...> whose Signature is a
    // function type of the form void(Args...). The partial specialisation
    // matches only that shape, so both "not an event_tag_config", "Signature is
    ...
    // callbacks::delegate_registry's void-return static_assert.
    // -------------------------------------------------------------------------
    template <typename T>
    ...
    // -------------------------------------------------------------------------
    // Extract per-config traits. delegate_dispatcher only looks at event_tag,
    ...
    //
    // Two event_tag_config entries with the same event_tag would create two
    ...
    // -------------------------------------------------------------------------
    // Per-event registry type — one non-owning callbacks::delegate_registry sized by
...
public:
    ...
    // event_tag_config<Tag, N, Signature>. This is enforced by the type of the
    // callback pointer: registry_type::callback_type == callbacks::delegate_base<Signature>.
    //
    // The caller owns the lifetime of the callbacks::delegate_base object and must keep it
    ...
    subscription register_callback(CallbackPtr callback, error* out_error = nullptr) CASTLE_NOEXCEPT
    {
        ...
        // convertible to the exact callbacks::delegate_base<Signature>* expected by Tag.
    ...
    // Signature in event_tag_config<Tag, N, Signature>.
    //
    // Perfect-forwarded to callbacks::delegate_registry::invoke(Args...).
...
private:
    ...
    // event_tag_config bound to a specific tag — resolves via the tuple.
    template <typename Tag>
    using config_for = castle::tuple_element_t<index_of<Tag>(), castle::tuple<EventConfigs...>>;

    // Direct access to the callbacks::delegate_registry for a specific tag.
    template <typename Tag>
...
private:
    ...
    // its event_tag_config::max_callback and typed by its signature — this is the
...
#endif // CASTLE_EVENTS_DELEGATE_DISPATCHER_HPP
