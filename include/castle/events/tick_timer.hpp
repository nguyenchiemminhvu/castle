...
// -----------------------------------------------------------------------------
// tick_timer - fixed-capacity, heap-free, tick-driven software timer
// that OWNS its callbacks by value (via callbacks::function_registry).
//
// Model:
...
//   CallbackStorageSize      - callbacks::function SBO size per callback.
//   CallbackStorageAlignment - callbacks::function SBO alignment per callback.
//
// Usage:
...
public:
    ...
    // -------------------------------------------------------------------------
    // Register a timeout callback. Any callable convertible to callbacks::function
    // signature void() is accepted (function pointers, stateless lambdas,
    // stateful lambdas within the SBO budget, functor objects, etc.).
    //
    // Returns an callbacks::subscription. On failure the handle is
    ...
    void on_tick(tick_type elapsed_ticks = 1) noexcept
    {
        ...
        while (running_ && counter_ >= period_)
        {
            ...
            // Fire the timeout event. Callbacks run in subscription order.
            registry_.invoke();
