...
// built on top of callbacks::function_registry (owning callbacks).
//
// Configuration model:
...
//   - Every signal owns its own callbacks::function_registry<
...
//     signum lookup, a bitset test, and a walk of callbacks::function slots
...
//   The callbacks::function machinery itself is neutral: subscription only
...
private:
    ...
    // -------------------------------------------------------------------------
    // Extract per-config traits. signal_ipc_event uses every field:
    ...
    // -------------------------------------------------------------------------
    // Per-signal registry type — one owning callbacks::function_registry sized
...
public:
    ...
    //
    // The registry OWNS the callback (copied/moved into the callbacks::function
    ...
    // management. The callable must satisfy callbacks::function's size /
    // alignment constraints declared by signal_ipc_config for this signum —
    // checked by a static_assert inside callbacks::function.
    //
...
private:
    ...
    // and a walk of callbacks::function slots invoked via a stored function
    ...
    // callbacks::function's inline buffer — zero heap.
