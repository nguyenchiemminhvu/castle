...
public:
    ...
    // Builds the heap incrementally via push(); element order in the list has
    // no bearing on the resulting heap, only on tie-breaking among equals.
    basic_heap(initializer_list<T> list) CASTLE_NOEXCEPT : size_(0U), compare_()
    {
        CASTLE_ASSERT(list.size() <= N, CASTLE_ERROR_GENERIC("basic_heap: initializer_list exceeds capacity")); // LCOV_EXCL_BR_LINE
