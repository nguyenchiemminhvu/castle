...
public:
    ...
    // Duplicate keys are ignored, matching std::set's initializer_list ctor.
    set(initializer_list<key_type> list) CASTLE_NOEXCEPT : tree_()
    {
        CASTLE_ASSERT(list.size() <= N, CASTLE_ERROR_GENERIC("set: initializer_list exceeds capacity")); // LCOV_EXCL_BR_LINE
...
#endif // CASTLE_CONTAINER_SET_HPP
