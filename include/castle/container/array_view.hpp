...
// Bridges a brace-init-list straight to a non-owning view: the backing array
// of an initializer_list is a compiler-managed temporary, so the returned
// view is only valid for the lifetime of that temporary (e.g. as a function
// argument, not stored past the full expression).
template <typename T>
...
#endif // CASTLE_CONTAINER_ARRAY_VIEW_HPP
