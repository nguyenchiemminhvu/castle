...
public:
    ...
    // Later duplicate keys are ignored, matching std::unordered_map's
    // initializer_list ctor.
    hash_table(initializer_list<value_type> list) CASTLE_NOEXCEPT
        : size_(0U), hash_(), equal_(), states_{}
    {
        CASTLE_ASSERT(list.size() <= N, CASTLE_ERROR_GENERIC("hash_table: initializer_list exceeds capacity")); // LCOV_EXCL_BR_LINE
