...
public:
    ...
    // Elements are appended in list order, matching std::forward_list's
    // initializer_list ctor.
    forward_list(initializer_list<T> list) CASTLE_NOEXCEPT
        : head_(nullptr), size_(0U), free_head_(nullptr), before_()
    {
        initialise_pool();
        CASTLE_ASSERT(list.size() <= N, CASTLE_ERROR_GENERIC("forward_list: initializer_list exceeds capacity")); // LCOV_EXCL_BR_LINE
