...
public:
    ...
    // Elements are pushed in list order, so top() == the list's last element (LIFO).
    stack_buffer(initializer_list<T> list) CASTLE_NOEXCEPT
    {
        CASTLE_ASSERT(list.size() <= N, CASTLE_ERROR_GENERIC("stack_buffer: initializer_list exceeds capacity")); // LCOV_EXCL_BR_LINE
...
private:
    ...
    // advance()/retreat() only ever move by one slot from an already-valid
    // index in [0, N), so a compare-and-reset is enough here: this avoids
    // the integer division that wrap()'s generic modulo needs whenever N is
    // not a power of two, which is far costlier than a well-predicted branch.
...
#endif // CASTLE_CONTAINER_STACK_HPP
