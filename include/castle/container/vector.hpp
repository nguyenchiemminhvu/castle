...
public:
    ...
    // Brace-init support: castle::container::initializer_list<T> is the type
    // the compiler itself fills in for `vector<T,N> v{a, b, c};`. Capacity is a
    // runtime property of the list, so an oversized list is a programmer error
    // reported through CASTLE_ASSERT rather than silently truncated.
    vector(initializer_list<T> list) : size_(0U)
    {
        static_assert(meta::is_copy_constructible<T>::value,
                      "vector initializer_list construction requires a copy-constructible T");
        CASTLE_ASSERT(list.size() <= N, // LCOV_EXCL_BR_LINE
                      CASTLE_ERROR_GENERIC("vector: initializer_list exceeds capacity"));
