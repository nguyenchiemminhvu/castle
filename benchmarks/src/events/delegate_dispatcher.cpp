...
using castle_dispatcher = castle::events::delegate_dispatcher<
    castle::events::event_tag_config<TickEvent, 4U, void(std::uint32_t)>>;
...
BENCHMARK_F(event_dispatch, castle)(benchmark::State& state)
{
    ...
    castle::callbacks::delegate_ptr_ct<&on_tick> callback;
    std::array<castle::callbacks::subscription, 4U> subscriptions{};
