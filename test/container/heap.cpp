...
TEST(HeapTest, RemoveAtSiftsUpWhenReplacementBeatsParent)
{
    ...
    EXPECT_EQ(h.push(111),castle::status::full);
    EXPECT_EQ(h.emplace(111),castle::status::full);
...
TEST(HeapTest, InitializerListConstructor)
{
    castle::container::min_heap<int,4> h{5,1,9,3};
    EXPECT_EQ(h.size(),4U);
    EXPECT_TRUE(h.full());
    int out=0;
    EXPECT_EQ(h.pop(out),castle::status::ok);
    ...
    castle::container::max_heap<int,4> mh{5,1,9,3};
    EXPECT_EQ(mh.top(),9);

    castle::container::min_heap<int,4> partial{2,1};
    EXPECT_EQ(partial.size(),2U);
    EXPECT_EQ(partial.top(),1);
}
} // namespace
