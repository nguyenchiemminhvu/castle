...
{
    using D=castle::events::delegate_dispatcher<
        castle::events::event_tag_config<A,2,void(int)>,
        castle::events::event_tag_config<B,1,void()>>;

    static_assert(D::event_capacity()==2U,"events");
    static_assert(D::callback_capacity<A>()==2U,"cap");
    D d;
    castle::callbacks::delegate_ptr<void(int)> ca(&fa);
    castle::callbacks::delegate_ptr<void()> cb(&fb);
