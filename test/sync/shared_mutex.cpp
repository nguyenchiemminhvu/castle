...
// ---------------------------------------------------------------------------
// castle::shared_mutex defaults to castle::spin_wait, castle's
// zero-dependency, bare-metal-safe WaitPolicy.
// ---------------------------------------------------------------------------
TEST(SharedMutexTest, WaitPolicyDefaultsToSpinWait)
{
    static_assert(castle::is_same<castle::shared_mutex<4>::wait_policy_type, castle::spin_wait>::value,
...
// lock_write(), instead of castle::spin_wait's default CPU-relax hint.
// ---------------------------------------------------------------------------
...
TEST(SharedMutexTest, CustomWaitPolicyInvokedWhileBlocked)
{
    counting_wait_policy::hit_count.store(0U);

    castle::shared_mutex<4, counting_wait_policy> m;
