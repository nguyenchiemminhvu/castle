...
int main()
{
    ...
    // ── forward iteration (logical order, oldest -> newest) ──────────────────
    // *(end() - 1) is the top of the stack.
    std::cout << "range-for  :";
    ...
    const auto found = std::find(buffer.begin(), buffer.end(), 40);
    std::cout << "std::find(40)   = ";
    if (found != buffer.end())
    {
    ...
    const auto min_it = std::min_element(buffer.begin(), buffer.end());
    std::cout << "std::min_element= " << *min_it << std::endl;
