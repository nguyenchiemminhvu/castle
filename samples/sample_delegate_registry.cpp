/**
 * Castle 2.0 sample: <castle/callbacks/delegate_registry.h>
 *
...
int main()
{
    using callback_type = castle::callbacks::delegate_ptr<void(uint32_t)>;
    using registry_type = castle::callbacks::delegate_registry<4U, void(uint32_t)>;

    callback_type callback{on_telemetry};
    registry_type registry;
    registry_type::error error = castle::status::ok;

    castle::callbacks::subscription subscription = registry.subscribe(&callback, &error);
